...
/**
 * Graph which implements {@code GraphInterface}. Will implement a directed Graph
 * data structure
 * 
 * @author low101043
 *
...
public class GraphOriginal implements GraphInterface {
	...
	public void addConnection(int originNode, int destinationNode, double weight) {
		...
		List<Connection> edgeList = graph.get(originNode);  //Gets the connection if in the graph.  If not there gets null
		...
		edgeList.add(new Connection(originNode, destinationNode, weight));  //adds a new connection to the set of connections
	...
	public void removeNode(int nodeToRemove) {
		...
		for (Map.Entry<Integer, ArrayList<Connection>> entry : graph.entrySet()) {  //Takes each pair of values in the graph
			
			int node = entry.getKey();  //Gets the node 
 
			for (Connection edge : entry.getValue()) {   //Takes each connection foor that node
				
				if (edge.getDestinationNode() == nodeToRemove) {  //If the destination node is equal to the node to remove it will be removed
				
					double weight = edge.getWeight();   //Gets the weigh
					Object[] connectionToRemove = { node, nodeToRemove, weight };  //The array which has the values to be removed
		...
		int sizeOfLinkedList = dataToRemove.size();  //Finds the size and then goes through the linked list of data items
	...
	public Object[][] getConnections() {
		...
		for (Map.Entry<Integer, ArrayList<Connection>> entry : graph.entrySet()) {  //For each pair of values in the hash map
